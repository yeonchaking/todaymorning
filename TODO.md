...
- [ ] **오류·예외 처리 표준화** — 도착정보 없음 / 네트워크 실패 / 운행종료 시간대의 사용자 안내. HttpException 공통 매핑 유틸. (2026-07-06 세션: 케이스 조사·구현 방향은 `오류_예외처리_표준화_케이스정리.md`에 정리 완료, 실제 코드 작업은 Phase 2로 이동.)
...
- [ ] **엣지케이스** — 미션 미설정 상태로 알람(확인 결과 이미 정상 동작), 자정 넘김(`MissionEngine.startCountdown` 롤오버 버그 확인 — 실제 수정 필요), 목표 시각 직전 진입(확인 결과 이미 정상 동작) 등. 조사 결과·수정 방향은 `엣지케이스_정리.md`에 정리 완료, 실제 코드 작업(자정 넘김)은 Phase 2로 이동.
