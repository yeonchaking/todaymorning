...
- [ ] **막차/첫차 계산 → 실시간 도착정보 반영** `[이번 릴리즈]` — 노선별 첫차·막차 시각을 가져와(버스: 공공API 노선정보 firstBusTm/lastBusTm, 지하철: 시간표) 현재 시각과 비교. 막차 끝난 상태면 도착정보 대신 "운행이 종료된 버스입니다" 같은 안내, 첫차 전이면 "아직 운행 전" 안내. 메인 실시간 도착정보 다이얼로그 + 타임어택 화면 모두 반영.
- [ ] **오류·예외 처리 표준화** `[이번 릴리즈]` — 도착정보 없음 / 네트워크 실패 / 운행종료 시간대의 사용자 안내. HttpException 공통 매핑 유틸. (2026-07-06 세션: 케이스 조사·구현 방향은 `오류_예외처리_표준화_케이스정리.md`에 정리 완료, 실제 코드 작업은 Phase 2로 이동.)
...
- [ ] **엣지케이스 (자정 넘김 롤오버 버그)** `[이번 릴리즈]` — 미션 미설정 상태로 알람(확인 결과 이미 정상 동작), 자정 넘김(`MissionEngine.startCountdown` 롤오버 버그 확인 — 실제 수정 필요), 목표 시각 직전 진입(확인 결과 이미 정상 동작) 등. 조사 결과·수정 방향은 `엣지케이스_정리.md`에 정리 완료, 실제 코드 작업(자정 넘김)은 Phase 2로 이동.
...
- [ ] **온보딩/첫 실행 화면** `[이번 릴리즈]` — 없음. 앱 목적·권한 설명·첫 미션 설정 유도. (2026-07-10: 1.0 포함 확정.)
...
- [ ] **플로팅 위젯 주석처리 (기능 비활성화)** `[이번 릴리즈]` — 1.0에서 위젯 제외(2026-07-10 결정). 오버레이 구현 주석처리 + Manifest `SYSTEM_ALERT_WINDOW` 권한 제거(민감 권한 선언 부담 제거).
