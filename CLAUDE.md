...
### 세션 시작 시
1. `SESSION_LOG.md`를 읽고 **맨 위 항목의 TODO**만 살아있는 할 일로 본다 (아래 항목 TODO는 역사적 기록)
...
### TODO 해석/관리 정책
- 살아있는 TODO는 **맨 위 항목의 것만**이다. 아래 항목 TODO는 끌어다 쓰지 않는다.
- TODO를 다음 항목에 **복붙하지 않는다.** 이전 TODO를 이번 세션에 처리했으면 그 사실로 닫고, 안 했으면 의도적으로 다시 적는다.
- TODO 착수 전 **현재 코드와 대조해 유효성을 확인**한다 (시간이 지나 stale 가능).
- 남은 TODO가 없으면 **`- 없음`** 으로 명시적으로 닫는다.
