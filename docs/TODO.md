...
- [x] ~~**클로즈드(비공개) 테스트 테스터 20명 모집**~~ — 해결(2026-07-18). 크몽 TechDive 대행 서비스 이용. 버전 5(1.4)를 비공개 테스트 Alpha 트랙에 업로드 → 콘솔 선언 폼 3종(FGS mediaPlayback·dataSync+시연영상 / FSI / 정확한 알람) 입력 → 심사 통과(출시됨). 판매자 제공 계정 27명을 "kmong 테스터" 이메일 목록으로 트랙에 등록. 14일 테스트는 판매자 진행 → 종료 후 프로덕션 신청. 상세는 `구글 플레이 콘솔 업로드 준비(오늘도출근).md` 1번.
